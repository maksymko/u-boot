 ...
 */
#define RGMII_TXCLK_ODLY		2
#define RMII_RXCLK_IDLY		8
 ...
 * 	need to be set to their maximum allowed values. The result (the best config we could find),
 * 	would also be returned in this structure.
 *
...
{
	ulong clkin = ast2500_get_clkin(scu);
	ulong hpll_rate = ast2500_get_hpll_rate(clkin, readl(&scu->h_pll_param));
	ulong required_rate;
	u32 hwstrap;
	u32 divisor;
	u32 reset_bit;
...
{
	/*
	 * The value for the constants and the meaning of the next three parameters
	 * are undocumented. Taken from Aspeed SDK.
	 */
	const u32 d2_pll_ext_param = 0x2c;
	const u32 d2_pll_sip = 0x11;
	const u32 d2_pll_sic = 0x18;
	u32 clk_delay_settings = (RMII_RXCLK_IDLY << SCU_MICDS_MAC1RMII_RDLY_SHIFT)
	...
	writel((d2_pll_ext_param << SCU_D2PLL_EXT1_PARAM_SHIFT)
		   | SCU_D2PLL_EXT1_OFF
		   | SCU_D2PLL_EXT1_RESET,
		   &scu->d2_pll_ext_param[0]);
	...
	writel((d2_pll_sip << SCU_D2PLL_SIP_SHIFT)
		   ...
		   | (div_cfg.post_div << SCU_D2PLL_POST_SHIFT), &scu->d2_pll_param);

	clrbits_le32(&scu->d2_pll_ext_param[0], SCU_D2PLL_EXT1_OFF | SCU_D2PLL_EXT1_RESET);

	clrsetbits_le32(&scu->misc_ctrl1,
					SCU_MISC2_RGMII_HPLL | SCU_MISC2_RMII_MPLL
					| SCU_MISC2_RGMII_CLKDIV_MASK | SCU_MISC2_RMII_CLKDIV_MASK,
					(4 << SCU_MISC2_RMII_CLKDIV_SHIFT));

	writel(clk_delay_settings | SCU_MICDS_RGMIIPLL, &scu->mac_clk_delay);

	/*
	 * FIXME: Not sure if the next two writes are necessary,
	 * they are not enabling the delay
	 */
...
static ulong ast2500_clk_set_rate(struct clk *clk, ulong rate)
{
	...
	case PLL_D2PLL:
		...
		break;
	/*
	 * For MAC clocks the requested rate is ignored and the clock rate is
	 * configured based on whether RGMII or RMII mode has been selected
	 * trhough hardware strapping.
	 */
	case PCLK_MAC1:
		new_rate = ast2500_configure_mac(priv->scu, 1);
		break;
	case PCLK_MAC2:
		new_rate = ast2500_configure_mac(priv->scu, 2);
		break;
	default:
		return -ENOENT;
	}
