...
static int ast_timer_probe(struct udevice *dev)
{
	...
	struct ast_timer_counter *tmc =
	    ast_get_timer_counter(priv->regs, AST_TICK_TIMER);
	 ...
	 * the status register.
	 */
	clrbits_le32(&priv->regs->ctrl1, AST_TMC_CTRL1_EN(AST_TICK_TIMER));
	/* Start the timer from the fixed 1MHz clock. */
	setbits_le32(&priv->regs->ctrl1,
		     AST_TMC_CTRL1_EN(AST_TICK_TIMER) |
		     AST_TMC_CTRL1_1MHZ(AST_TICK_TIMER));
