...
int do_mac(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	uint32_t mac_low3 = (fru_iua.mac_addr[3] << 16)
			| (fru_iua.mac_addr[4] << 8)
			| (fru_iua.mac_addr[5]);
	int i = 0;
	for (; i < fru_iua.num_mac_addrs; ++i) {
		printf("#%d: %02x:%02x:%02x:%02x:%02x:%02x\n", i + 1,
			   ...
			   fru_iua.mac_addr[1],
			   fru_iua.mac_addr[2],
			   (uint8_t)((mac_low3 >> 16) & 0xff),
			   (uint8_t)((mac_low3 >> 8) & 0xff),
			   (uint8_t)(mac_low3 & 0xff));
		++mac_low3;
	}
	return 0;
}
...
int mac_read_from_eeprom(void)
{
	...
	for (; i < fru_iua.num_mac_addrs; ++i) {
		...
		++mac_low3;
		fru_mac_addr[5] = mac_low3 & 0xff;
		fru_mac_addr[4] = (mac_low3 >> 8) & 0xff;
		fru_mac_addr[3] = (mac_low3 >> 16) & 0xff;
	}
