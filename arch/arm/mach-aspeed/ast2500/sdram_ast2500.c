...
static size_t ast2500_sdrammc_get_vga_mem_size(struct dram_info *info)
{
	...
	u32 vga_hwconf = (readl(&info->scu->hwstrap) >> SCU_HWSTRAP_VGAMEM_SHIFT)
...
static void ast2500_sdrammc_calc_size(struct dram_info *info)
{
	...
	for (ram_size = SDRAM_MAX_SIZE; ram_size > SDRAM_MIN_SIZE; ram_size >>= 1) {
	...
	clrsetbits_le32(&info->regs->config,
			...
			((cap_param & SDRAM_CONF_CAP_MASK) << SDRAM_CONF_CAP_SHIFT));
}
